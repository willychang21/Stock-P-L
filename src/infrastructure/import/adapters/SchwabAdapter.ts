...
export class SchwabAdapter implements BrokerAdapter {
  ...
  parseRow(row: Record<string, string>): RawTransaction | null {
    ...
    const skipActions = ['JOURNAL', 'BANK INTEREST', 'WIRE', 'TRANSFER', 'ACH'];
    if (skipActions.some(skip => action.includes(skip))) {
      throw new Error(`Skipping non-position action: ${action}`);
    }
    ...
    const settleDate = this.parseDate(row['Settle Date'] || row['Settlement Date']);
    ...
    if (transactionType === TransactionType.DIVIDEND) {
        if (new Decimal(quantity).isZero() && !new Decimal(amount).isZero()) {
  ...
  private getSymbol(row: Record<string, string>): string {
    return (this.getValue(row, ['Symbol', 'Ticker', 'Instrument']) || '').toUpperCase().trim();
  }
  ...
  private getTransactionType(row: Record<string, string>): TransactionType {
    ...
    // BUY actions
    if (action.includes('BUY') || action.includes('BOUGHT') || action.includes('REINVEST SHARES')) {
  ...
  private parseFees(row: Record<string, string>): string {
    ...
    const feesComm = this.getValue(row, ['Fees & Comm', 'Fees', 'Commission']) || '0';
